'''
version: light
description: target identification of person, animals, etc
date: 2020-03-18
author: jtx
'''
...
def yolo_detect(pathIn='',
                pathOut=None,
                label_path='00yolo/coco.names',
                config_path='00yolo/yolov3.cfg',
                weights_path='00yolo/yolov3.weights',
                confidence_thre=0.5,
                nms_thre=0.3,
                jpg_quality=80):
    '''
    ...
    # 加载模型配置和权重文件
    '''从磁盘加载YOLO文件后，并利用OpenCV中的cv2.dnn.readNetFromDarknet函数从中读取网络文件及权重参数，
    此函数需要两个参数configPath 和 weightsPath，这里再次强调，：OpenCV 的版本至少是3.4.2及以上才能运行此代码，
    因为它需要加载YOLO所需的更新的 dnn模块'''
    print('从硬盘加载YOLO......')
